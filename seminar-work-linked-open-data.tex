...
\paragraph{Semantically enriched web pages:}

\paragraph{Linked Open Data:} 
...
\paragraph{Data acquisition} is the first step of the integration process and is required for acquiring the data that shall be integrated into the system. According to Simperl et.al \cite{simperl_using_2013} there are number of ways, in which the data consumed and integrated within the system may be accessed. The approach used for this proposed architecture is the }\textit{crawling pattern}, where the data is loaded in advance and stored in a local triple store so that it can be accessed efficiently. \enquote{The disadvantage of this pattern is that the data might not be up to date}. An alternative approach is the \textit{On-The-Fly Dereferencing pattern}, \enquote{where the URIs are dereferenced at the moment that the system requires the data}. The advantage is up-to-data data, but the performance may be affected when the data transformation and dereferencing is costly.\cite{simperl_using_2013}.
...
In the \textit{\textbf{integration tier}} the demanded and available datasets were integrated into a local triple store. The \textit{\textbf{provision tier}} gives now an overview of the different ways to grant the end user access to the linked data stored in the local triple store. The advantage of SPARQL endpoints is that the end user can flexibly request a particular subset of the data stored in the local triple store.

\paragraph{SPARQL endpoint:} SPARQL is a language that can be used to query a RDF dataset or in this case the whole triple store. A SPARQL endpoint is a web service that can process SPARQL requests and returns the corresponding result.
...
In order to not violate the principles of Linked Open Data the API must be open. Linked data API\footnote{\url{http://www.epimorphics.com/web/projects/linked-data-api}} can be used to provide a RESTful web service to the end user making also SPARQL requests possible.

\paragraph{CMS enriched with RDFa:}
